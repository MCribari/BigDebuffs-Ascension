...
local GetAnchor = {
    ...
    KkthnxUIFrames = function(anchor)
...
local anchors = {
    ...
    ["KkthnxUI"] = {
        func = GetAnchor.KkthnxUIFrames,
        units = {
            player = "oUF_Player",
            pet = "oUF_Pet",
            target = "oUF_Target",
            focus = "oUF_Focus",
            party1 = "oUF_PartyUnitButton1",
            party2 = "oUF_PartyUnitButton2",
            party3 = "oUF_PartyUnitButton3",
            party4 = "oUF_PartyUnitButton4",
            party5 = "oUF_PartyUnitButton5",
            arena1 = "oUF_Arena1",
            arena2 = "oUF_Arena2",
            arena3 = "oUF_Arena3",
            arena4 = "oUF_Arena4",
...
function BigDebuffs:Refresh()
    ...
    for unit, frame in pairs(self.Nameplates) do
        ...
        if self.db.profile.unitFrames.cooldownCount then
            local text = frame.cooldown.text or frame.cooldown:GetRegions()
            local fontPath = LibSharedMedia:Fetch("font", BigDebuffs.db.profile.unitFrames.cooldownFont)
            local fontSize = self.db.profile.unitFrames.cooldownFontSize or 12
            local fontFlags = self.db.profile.unitFrames.cooldownFontEffect or ""

            if fontPath and text and text.SetFont then
                text:SetFont(fontPath, fontSize, fontFlags)
            else
                DEFAULT_CHAT_FRAME:AddMessage("BigDebuffs: Font nenalezen nebo neplatnÃ½ objekt textu")
...
function BigDebuffs:AttachUnitFrame(unit)
    if InCombatLockdown() and not self.test then return end
...
function BigDebuffs:UNIT_AURA(unit)
	...
	local unitConfig = self.db.profile.unitFrames[unitKey]

	if not unitConfig then
		return
	end

	if not unitConfig.enabled then
		return
	end
	...
	if guid and self.units[guid] and self.units[guid].expires and self.units[guid].expires > now then
		...
		if spellId and self.Spells[spellId] then
			...
			if p and p >= priority then
				...
				duration = self.Spells[spellId].duration or 0
				debuff = spellId
...
function BigDebuffs:NAME_PLATE_UNIT_ADDED(_, unit)
    ...
    if not frame.BigDebuffs then
        ...
        -- Fake cooldown frame
        local cooldown = CreateFrame("Frame", "$parent.Cooldown", frame.BigDebuffs)
        cooldown:SetAllPoints(frame.BigDebuffs)
        ...
        if fontPath then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("BigDebuffs: Font nenalezen, pouÅ¾Ã­vÃ¡m vÃ½chozÃ­.")
            cooldown.text:SetFont("Fonts\\FRIZQT__.TTF", fontSize, fontFlags)
        end
        ...
        cooldown:SetScript("OnUpdate", function(self, elapsed)
            if self.enabled and self.start and self.duration then
                ...
                if remaining > 0 then
                    ...
                    elseif remaining < 10 then
                        ...
                        self.text:SetTextColor(0.8, 0.8, 0.2)
                    end
        ...
        -- Compat
        function cooldown:SetHideCountdownNumbers(hide)
            self.text:SetShown(not hide)
        end

        cooldown.noCooldownCount = false

        frame.BigDebuffs.cooldown = cooldown
    end
